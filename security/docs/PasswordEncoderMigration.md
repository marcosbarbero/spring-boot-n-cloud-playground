...
[bcrypt](https://en.wikipedia.org/wiki/Bcrypt). The current passwords were stored all as `hash` which means it was not
possible to reserve it to the original `String` - at least not in an easy way.
